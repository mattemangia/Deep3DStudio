...
# =============================================================================
# CRITICAL: Mock CUDA-only modules BEFORE any other imports
# This must happen before torch imports to prevent spconv/flash_attn failures
# =============================================================================
def setup_early_cuda_mocks():
    """
    ...
    # Create mock spconv module with all necessary components
    spconv_mock = MagicMock()
    spconv_pytorch_mock = MagicMock()

    # Import torch.nn for creating proper nn.Module subclasses
    import torch
    ...
    class MockSparseMaxPool3d(nn.Module):
        ...
        def __init__(self, kernel_size, stride=None, padding=0, dilation=1, *args, **kwargs):
            super().__init__()
    ...
    # Assign all components to mock modules
    ...
    print("  ✓ spconv mocked (SparseConv3d, SubMConv3d, SparseConvTensor, etc.)")
    ...
    print("  ✓ triton, apex mocked")
    print("=" * 60)
    print("CPU-only mocks installed successfully!")
    print("=" * 60 + "\n")

# Call early mocks IMMEDIATELY
setup_early_cuda_mocks()
