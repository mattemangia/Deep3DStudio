            ...
            try
            {
                ...
                else
                {
                    _lastError = _inference.LastOpenMpShmError
                        ? "Failed to load DUSt3R model due to OpenMP shared-memory initialization on macOS (OMP #179)."
                        : (_inference.LastErrorMessage ?? "Failed to load DUSt3R model");
            ...
            try
            {
                ...
                foreach (var path in imagePaths)
                {
                    ...
                    else
                    {
                        ...
                        Log($"[Dust3r] Warning: Could not read image size for {Path.GetFileName(path)}");
                    }
                }
                ...
                for (int i = 0; i < meshes.Count; i++)
                {
                    ...
                    {
                        Log($"[Dust3r] Applied pose to point cloud {i}: localScore={localScore:F2}, worldScore={worldScore:F2}");
                    }
