...
def is_macos_target(target_platform):
    target = (target_platform or "").lower()
    return "osx" in target or "mac" in target


def get_macos_openmp_env_patch():
    """
    Runtime environment patch for macOS OpenMP/PyTorch stability.
...
def apply_macos_runtime_patch(site_packages, target_platform):
    """
    Install a sitecustomize.py runtime patch for macOS deployments.
    It runs before user imports and sets safe OpenMP defaults.
    """
    if not is_macos_target(target_platform):
        return
    ...
    patch_content = textwrap.dedent("""\
        # Auto-generated by setup_deployment.py (macOS-only runtime patch).
        # Loaded automatically by Python site initialization.
        import os
        import sys
...
def apply_macos_mast3r_must3r_patch(site_packages, target_platform):
    """
    Apply a macOS-only compatibility patch for DUSt3R / MASt3R / MUSt3R layout expectations.
    Linux/Windows paths are left untouched.
    """
    if not is_macos_target(target_platform):
        return
    ...
    print("Applying macOS DUSt3R/MASt3R/MUSt3R compatibility patch...")

    dust3r_pkg = os.path.join(site_packages, "dust3r")
    if os.path.isdir(dust3r_pkg):
        compat_submodule = os.path.join(dust3r_pkg, "dust3r")
        os.makedirs(compat_submodule, exist_ok=True)
        compat_init = os.path.join(compat_submodule, "__init__.py")
        if not os.path.exists(compat_init):
            with open(compat_init, "w", encoding="utf-8") as f:
                f.write("# Auto-generated macOS compatibility shim for MASt3R/MUSt3R.\n")
                f.write("from dust3r import *\n")
            print(f"  Created shim: {compat_init}")

    for package_name in ("mast3r", "must3r"):
        utils_dir = os.path.join(site_packages, package_name, "utils")
        os.makedirs(utils_dir, exist_ok=True)
        patch_path = os.path.join(utils_dir, "path_to_dust3r.py")
        patch_content = textwrap.dedent("""\
            # Auto-generated by setup_deployment.py (macOS-only compatibility patch).
            # This removes hardcoded repo layout assumptions from pip installs.
            import os
            import sys

            _SITE_PACKAGES = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
            DUSt3R_REPO_PATH = _SITE_PACKAGES
            DUSt3R_LIB_PATH = os.path.join(_SITE_PACKAGES, "dust3r")

            if DUSt3R_REPO_PATH not in sys.path:
                sys.path.insert(0, DUSt3R_REPO_PATH)
            if DUSt3R_LIB_PATH not in sys.path:
...
def setup_models(models_dir, python_dir, target_platform):
    ...
    apply_macos_mast3r_must3r_patch(site_packages, target_platform)
